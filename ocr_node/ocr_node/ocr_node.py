    ...
    def image_callback(self, msg):
        ...
        if results and results[0] is not None:
            for line in results[0]:
                ...
                # Set Hypothesis (Text and Confidence)
                hyp = ObjectHypothesisWithPose()
                hyp.hypothesis.class_id = str(text)  # Text goes here
                hyp.hypothesis.score = float(confidence) # Score goes here
                det_msg.results.append(hyp)
                
                det_array_msg.detections.append(det_msg)

                # 2. Draw on frame for visualization
